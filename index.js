...
function cumlelereDonustur(degisken, ayrac = ",") {
  const string = [...degisken];
  const render = string.map((item) => {
    const sonuc = item.join(ayrac);
    return sonuc;
  });
  return render;
}
...
function paragrafOlustur(degisken1, degisken2, degisken3) {
  const degisen1 = degisken1;
  const degisen3 = [...degisken3(degisen1, " ")];
  const degisen2 = degisken2(
    degisen3[1],
    degisen3[3],
    degisen3[5],
    degisen3[7],
 ...
 */
// napıyorsun kanka :D ben yeni geldim
// :D :(
function emojileriDonustur(string, ifade) {
  let deger = string.split(" ");
  const keys = Object.keys(ifade);
  const render = keys.forEach((item) => {
    let buyukHarf = item.toUpperCase();
    for (let i = 0; i < deger.length; i++) {
      if (item === deger[i] || buyukHarf === deger[i]) {
        deger[i] = ifade[item];
      }
    }
  });
	const total = deger.join(" ");
	return total;
}
// function emojileriDonustur(mesaj, emojiler) {
//   Object.entries(emojiler).forEach(([key, value]) => {
//     mesaj = mesaj.toLowerCase().replaceAll(key.toLowerCase(), value);
//   });
//   return mesaj;
// }
//   if (deger.includes(item)) {
//   let index = string.indexOf(item);
//   deger[index] = ifade[item];
// }
